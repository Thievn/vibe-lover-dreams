...
export type XMarketingPortraitTier = "selfie" | "lewd" | "nude";
...
export function loadPortraitTierForCompanion(companionId: string): XMarketingPortraitTier {
  try {
    ...
    if (v === "lewd" || v === "nude" || v === "selfie") return v;
  } catch {
    /* ignore */
  }
  return "selfie";
...
export const X_MARKETING_PORTRAIT_TIERS: { id: XMarketingPortraitTier; label: string; hint: string }[] = [
  ...
  },
  {
    id: "nude",
    label: "Nude / explicit",
    hint: "Best matching explicit generated image. Falls back to catalog if none. X media rules are your responsibility.",
  },
];

type GenRow = { image_url: string; prompt: string | null; created_at?: string };

function scoreNudePrompt(prompt: string | null): number {
...
function scoreLewdPrompt(prompt: string | null): number {
  ...
  // Soft NSFW without going full explicit still counts a little
  if (/\btopless\b/.test(p)) s += 1;
  const n = scoreNudePrompt(prompt);
  if (n > 0) s += Math.min(3, n * 0.35);
  return s;
}

/**
 * Picks the best `generated_images` row for marketing hero when tier is lewd or nude.
 * Rows should be newest-first; tie-break keeps newer images.
 */
export function pickBestGeneratedImageForTier(rows: GenRow[], tier: "lewd" | "nude"): string | null {
  ...
  for (let idx = 0; idx < rows.length; idx++) {
    const r = rows[idx]!;
    const sc = tier === "nude" ? scoreNudePrompt(r.prompt) : scoreLewdPrompt(r.prompt);
...
export function resolvePortraitHeroUrlForX(
  ...
  const picked = pickBestGeneratedImageForTier(generatedRows ?? [], tier);
...
export function portraitTierForGrokLine(tier: XMarketingPortraitTier): string {
  switch (tier) {
    ...
    case "nude":
      return "Hero visual: explicit / nude-aligned art from gallery when available. Copy: implication and CTA; do not describe genitals or explicit acts in the tweet text.";
    default:
      return "";
  }
}
