...
export const CHAT_LIKENESS_SCENE_PRIMACY_FOOTER =
  "**Scene primacy (non-negotiable):** The block under **Requested framing (from menu)** is the **sole** authority for **location, background, architecture, time of day, weather, furniture, wardrobe, pose, props, interaction with props, lens height, and camera distance**. For **typed** chat (no menu row), the **user message** carries the same authority for **environment**. The **Exposure / tone context** section sets SFW vs lewd vs artistic-nude **band only** — it must **never** replace the menu’s or user’s place/outfit/pose with a generic bedroom, bathroom mirror, studio bust, or catalog three-quarter. **Likeness channel:** When a roster portrait HTTPS URL appears in Character Details, use it **only** for " +
  CHAT_LIKENESS_SUBJECT_FEATURES_INLINE +
  "; **never** copy its backdrop, crop, pose, lighting recipe, or wardrobe unless this menu or user text explicitly matches that shot. **With no URL**, likeness = CHARACTER APPEARANCE + body type + species **from words only** — still **forbidden:** remastering the catalog packshot layout. Each preset must read as a **different photoshoot** in that **exact** scenario.";
