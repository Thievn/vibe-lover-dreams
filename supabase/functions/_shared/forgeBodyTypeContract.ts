...
function forgeBodyCategoryIdEdge(bodyType: string): string {
  ...
  if (
    /\b(cartoon\s*caricature|celebrity\s*caricature|\bchibi\b|pixel\s*art\s*character)\b/i.test(t)
  ) {
    return "creative";
  }
  if (
    /\b(little\s*person|midget|short\s*stature|tiny\s*&\s*doll|pixie-sized|micro\s*\/\s*tiny|giantess|giant\s*body|giant\s*\/\s*gargantuan|gargantuan|kaiju)\b/i.test(
      t,
    )
  ) {
    return "stature";
  }
  if (/\b(amputee|wheelchair|one-legged|one-armed|double\s*amputee|prosthetic|cybernetic)\b/i.test(t)) {
    return "mobility";
  }
  if (/\b(furry|anthro|wolf-anthro|feline-anthro|monster\s*girl|monster\s*boy|werewolf)\b/i.test(t)) {
    return "anthro";
  }
  if (/\b(tentacle|mermaid|centaur|harpy|naga|insectoid|arachnid|reverse\s*centaur)\b/i.test(t)) {
    return "hybrid";
  }
  if (
  ...
  if (
    /\b(living\s*doll|inflatable|slime|goo|gelatinous|crystal|ghostly|latex|melting\s*wax|shadow|plant-based|floating|mechanical|holographic|stone\s*\/|fire\s*elemental|ice\s*elemental|neon\s*wireframe|skeleton|wraith|golem|clockwork|ash|lightning|water-formed|living\s*object)\b/i.test(
      t,
    )
  ) {
    return "otherworldly";
...
function forgeBodyTypeImagineLeadEdge(bodyType: string): string {
  const raw = bodyType.trim() || "Average Build";
  return (
    FORGE_BODY_IMAGINE_LEADS[raw] ?? fallbackImagineLeadForCategory(forgeBodyCategoryIdEdge(raw), raw)
  );
}
...
export function forgePortraitBodyTypeContract(bodyType: string): string {
  const bt = bodyType.trim() || "Average Build";
  const cat = forgeBodyCategoryIdEdge(bt);
  const lead = forgeBodyTypeImagineLeadEdge(bodyType);
